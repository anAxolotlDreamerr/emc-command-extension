...
import io.github.anaxolotldreamerr.client.commands.cx.childcommand.resx.SetOption;
...
public class CResx implements EMCCommand {
    ...
    public EMCCommand register(CommandDispatcher<FabricClientCommandSource> dispatcher) {
        CommandNode<FabricClientCommandSource> resx = dispatcher.register(ClientCommandManager.literal("resx").executes(context -> {
            Minecraft minecraft = Minecraft.getInstance();
            minecraft.getConnection().sendCommand("favorites -p");
            return 0;
        }));
